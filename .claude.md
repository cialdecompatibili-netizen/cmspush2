...
## ⭐ PUBBLICAZIONE ARTICOLI/PRODOTTI — usa SEMPRE automation/

Per pubblicare un articolo o un prodotto in automatico su cmspush2, NON scrivere mai file .md a mano o script improvvisati. Leggere prima `automation\agent.md` e usare `automation\publish.py` (funzioni `pubblica_articolo` / `pubblica_prodotto`) — motore stabile gia' testato che gestisce slug, front-matter e git push da solo. Questa e' la procedura definitiva, sostituisce qualsiasi metodo manuale precedente.
